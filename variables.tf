...
variable "sbcli_cmd" {
  ...
  type        = string
  validation {
    condition     = can(regex("^sbcli-dev$|^sbcli-release$|^sbcli-mig$|^sbcli$|^sbcli-pre$", var.sbcli_cmd))
    error_message = "Invalid sbcli command. Please choose one of: sbcli-dev, sbcli-release, sbcli-mig, sbcli, sbcli-pre."
  }
}
